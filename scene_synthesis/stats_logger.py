...
class StatsLogger(object):
    ...
    def set_total_epochs(self, total_epochs):
        """Set the total number of epochs for ETA calculation."""
    ...
    def print_progress(self, epoch, batch, loss, precision="{:.5f}"):
        ...
        if epoch != self._current_epoch:
            if self._current_epoch > 0:
                # Calculate epoch time for the previous epoch
                current_time = time.time()
                if len(self._epoch_times) == 0:
                    # First epoch timing estimation
                    epoch_duration = current_time - self._start_time
                else:
                    # Calculate based on previous epoch end time
                    epoch_duration = current_time - (self._start_time + sum(self._epoch_times))
        ...
        # Add timing information if we have enough data
        if self._start_time and self._total_epochs and len(self._epoch_times) > 0:
