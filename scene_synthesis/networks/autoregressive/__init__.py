...
def hidden2output_layer(config, n_classes, n_dependency_classes=None):
...
def build_network(
    ...
    if network_type == "autoregressive_transformer":
        ...
        network = AutoregressiveTransformer(
            input_dims,
            hidden2output_layer(config, n_classes, n_dependency_classes),
    ...
    if weight_file is not None:
        ...
        # Remove unexpected keys that might cause loading errors
        # (e.g., position_ids that are now buffers instead of parameters)
        keys_to_remove = [k for k in state_dict.keys() if 'position_ids' in k]
        for key in keys_to_remove:
            print(f"Removing unexpected key from state_dict: {key}")
            del state_dict[key]
