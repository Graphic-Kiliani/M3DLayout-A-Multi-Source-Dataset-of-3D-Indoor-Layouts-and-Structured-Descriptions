...
class AutoregressiveDMLL(Hidden2Output):
    def __init__(
        ...
        self.dependency_layer = nn.Linear(hidden_size, n_dependency_classes) if n_dependency_classes else None
    ...
    def _extract_properties_from_target(sample_params):
        ...
        dependency_labels = sample_params.get("dependency_labels_tr", None)
        if dependency_labels is not None:
            dependency_labels = dependency_labels.float()
        return class_labels, translations, sizes, angles, dependency_labels
    ...
    def pred_class_probs(self, x):
        class_labels = self.class_layer(x)
        dependency_labels = self.dependency_layer(x) if self.dependency_layer else None
        ...
        dependency_probs = None
        if dependency_labels is not None:
            dependency_probs = torch.softmax(dependency_labels, dim=-1).view(b*l, -1)

        return class_probs, dependency_probs
    ...
    def forward(self, x, sample_params):
        ...
        dependency_labels = target_properties[4]
        ...
        a = self.pe_angle_z(angles)
        class_labels_pred = self.class_layer(x)
        dependency_labels_pred = self.dependency_layer(x) if self.dependency_layer else None
        ...
        return self.bbox_output(sizes, translations, angles, class_labels_pred, dependency_labels_pred)
